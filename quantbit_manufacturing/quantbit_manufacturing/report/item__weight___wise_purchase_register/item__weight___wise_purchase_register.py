	...
	for d in item_list:
		...
		row = {
			...
			**get_values_for_columns(additional_table_columns, d),
			...
			"value_per_unit":d.value_per_unit,
...
def get_items(filters, additional_query_columns):
	...
	return frappe.db.sql(
		"""
		select
		 `tabProduction UOM Definition`.value_per_unit * `tabPurchase Invoice Item`.`stock_qty` as  value_per_unit,
		...
		from `tabPurchase Invoice`, `tabPurchase Invoice Item`, `tabItem` ,`tabProduction UOM Definition`
		where `tabPurchase Invoice`.name = `tabPurchase Invoice Item`.`parent` and
			`tabItem`.name = `tabPurchase Invoice Item`.`item_code` and `tabItem`.name= `tabProduction UOM Definition`.parent and
