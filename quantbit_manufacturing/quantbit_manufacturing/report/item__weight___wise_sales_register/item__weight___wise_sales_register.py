...
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	return frappe.db.sql(
		"""
		select
			`tabProduction UOM Definition`.value_per_unit * `tabSales Invoice Item`.stock_qty as value_per_unit,
		...
		from `tabSales Invoice`, `tabSales Invoice Item`, `tabItem` , `tabProduction UOM Definition`
		where `tabSales Invoice`.name = `tabSales Invoice Item`.parent and
			`tabItem`.name = `tabSales Invoice Item`.`item_code` and `tabItem`.name= `tabProduction UOM Definition`.parent  and
